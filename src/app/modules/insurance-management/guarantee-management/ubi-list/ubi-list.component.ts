...
export class UbiListComponent extends MitDataTableBase implements OnInit, OnDestroy {
  ...
  // 获取ubi明细
  getDetail(data) {
    this._ubiDetail = this.guaranteeManagementService.GetUBIInsuranceItem(data).subscribe((res) => {
      if (res.Data) {
  ...
  getList() {
    ...
    this._actualUBIForTime = this.guaranteeManagementService.GetActualUBIForTime(this.query).subscribe((res) => {
      if (res.State) {
