...
export class CarLocationComponent implements OnInit, OnDestroy {
  ...
  public ubiHistoryStatus: boolean = false; // ubi历史保费显示状态
  public ubiRealTimeStatus: boolean = false; // ubi实时保费显示状态

  public realTimeInsuranceData: any;  // 实时保费数据
  public historyInsuranceData: any; // 历史保费数据
  ...
  EventProxy() {
    this.eventsService.getEmitter().subscribe((item) => {
      if (this.currentUrl === '/page/insurance-management/car-location/control/history-track' && item.data) {
        this.TrackLineData = item.data;
        this.ShowHistoryLine(item.data);
      } else if (this.currentUrl === '/page/insurance-management/car-location/control/real-time' && item.data) {
        this.ShowRealTimeLine(item.data);
      }
    });

    this.eventsService.getCommonEmitter().subscribe((item) => {
      if (item.name == 'realTimeInsurance') {
        this.realTimeInsuranceData = item.data;
      } else if (item.name == 'historyInsurance') {
        this.historyInsuranceData = item.data;
      }
    });
  ...
  currentPage(url) {
    ...
    switch (this.currentUrl) {
      case '/page/insurance-management/car-location/control/real-time': // 进入实时轨迹
        this.EnterRealTime();
        break;
      case '/page/insurance-management/car-location/control/history-track': // 进入历史轨迹
        this.EnterHistory();
        break;
      case '/page/insurance-management/car-location/control/single-vehicle/vehicle-info': // 进入单车面板
        this.EnterSingleCar();
        break;
      case '/page/insurance-management/car-location/control/single-vehicle/driver-info': // 进入单车面板
        this.EnterSingleCar();
        break;
      case '/page/insurance-management/car-location/control/single-vehicle/car-service-info': // 进入单车面板
        this.EnterSingleCar();
        break;
      case '/page/insurance-management/car-location/control/single-vehicle/statistics-info': // 进入单车面板
        this.EnterSingleCar();
        break;
      default: // 显示所有车辆
        this.EnterAllCar();
  ...
  OpenSingleVehicle(item) {
    // 存在Did时调整
    if (item.Did) {
      this.router.navigate(['/page/insurance-management/car-location/control', item.Did]);
    }
  }
  ...
  hiddenSome() {
    ...
    this.gradientPath = false;
    this.ubiHistoryStatus = false;
    this.ubiRealTimeStatus = false;
  }
