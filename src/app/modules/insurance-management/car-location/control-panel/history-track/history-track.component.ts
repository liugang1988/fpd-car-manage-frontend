...
export class HistoryTrackComponent implements OnInit, OnDestroy {
  ...
  public TrackDynamicPrenium: any;  // 历史行程保费
  ...
  // 选择事件
  selected(trackId, VID) {
    if (this.selectID !== trackId) {
      this.getTrack(trackId);
      this.getTrackDynamicPrenium({
        VehicleId: VID,
        TrackId: trackId
      })
    }
    this.selectID = trackId;
    this.isPlayCtrl = true;
    this.isSearchArea = false;
  }

  // 获取历史轨迹保费
  getTrackDynamicPrenium(data) {
    this.historyTrackService.GetTrackDynamicPrenium(data).subscribe((res) => {
      if (res.State) {
        this.eventsService.emitEvent('historyInsurance', res.Data);
      }
    }, (err) => {
      if (err.State == 10 || err.State == 11 || err.State == 12) {
        this.eventsService.emitMessageEvent(err.State == 1 ? this.eventsService.eventNames.EVENT_TOAST_SUCCESS : this.eventsService.eventNames.EVENT_TOAST_ERROR, err.Message);
