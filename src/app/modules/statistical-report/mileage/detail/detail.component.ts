...
export class DetailComponent extends MitDataTableBase implements OnInit, OnDestroy {
  public id:any;
  public getId:any;
  public singleMileageDetail: any; 
  public singleMileageChart: any;
  public chartNameList = [];
  public chartDataList = [];
  public singleMileage:any;
  ...
  // 获取详情
  getDetail(data) {
    this._getDetail = this.mileageService.MileageSummaryDetail(data).subscribe((res) => {
        if (res.State) {
          this.singleMileageDetail = res.Data;
        }
      },(err) => {
  ...
  // 获取图表
  getChart(data){
    this._getChart = this.mileageService.MileageSummaryForDay(data).subscribe((res) => {
        if (res.State) {
          this.singleMileageChart = res.Data;
          this.chartNameList = [];
          this.chartDataList = [];
          if(this.singleMileageChart){
            this.singleMileageChart.forEach((item) => {
              this.chartNameList.push(item.Date);
              this.chartDataList.push(item.TrackMileage);
            })
            this.showChart();
          }
        }
  ...
  getList() {
    ...
    this.rows = [];
    this._getList = this.mileageService.MileageSummaryForTrafficData(this.query).subscribe((res) => {
        if (res.State) {
  ...
  download() {
    ...
    this.text = '导出中...';
    this.isModal = true;
    this._download = this.mileageService.MileageSummaryForTrafficDataExcel(data).subscribe((res) => {
      const blob = new Blob([res], {type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"});  
      const objectUrl = URL.createObjectURL(blob); 
      if (window.navigator.msSaveOrOpenBlob) {
        navigator.msSaveBlob(blob, '里程统计明细' + this.StartTimeShow + '至' + this.EndTimeShow);
      } else {
        const a = document.createElement('a');
        document.body.appendChild(a);
        a.setAttribute('style', 'display:none');
        a.setAttribute('href', objectUrl);
        a.setAttribute('download','里程统计明细' + this.StartTimeShow + '至' + this.EndTimeShow);
        a.click();
