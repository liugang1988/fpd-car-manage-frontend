...
export class ListComponent extends MitDataTableBase implements OnInit, OnDestroy {
  ...
  getList() {
    ...
    this._getList_ = this.mileageService.MileageSummary(this.query).subscribe(
      (res) => {
        if (res.State) {
  ...
  download(KeyValue){
    ...
    this.text = '导出中...';
    this.isModal = true;
    this._download = this.mileageService.MileageSummaryExportExcel(data).subscribe((res) => {
        const blob = new Blob([res], {type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"});  
        const objectUrl = URL.createObjectURL(blob); 
        if (window.navigator.msSaveOrOpenBlob) {
          navigator.msSaveBlob(blob, '里程统计报表'+this.Time);
        } else {
          const a = document.createElement('a');
          document.body.appendChild(a);
          a.setAttribute('style', 'display:none');
          a.setAttribute('href', objectUrl);
          a.setAttribute('download','里程统计报表'+this.Time);
          a.click();
