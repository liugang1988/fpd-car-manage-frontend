...
export class ListComponent extends MitDataTableBase implements OnInit, OnDestroy {
  ...
  getList() {
    ...
    this._getDriverRank = this.driverRankService.GetPageDriverScores(this.query).subscribe((res) => {
      if (res.State) {
  ...
  download(KeyValue) {
    ...
    this.text = '导出中...';
    this.isModal = true;
    this._download = this.driverRankService.DriverScoresExportExcle(data).subscribe((res) => {
      const blob = new Blob([res], {type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"});  
      const objectUrl = URL.createObjectURL(blob); 
      if (window.navigator.msSaveOrOpenBlob) {
        navigator.msSaveBlob(blob, '驾驶员评分统计报表' + this.Time);
      } else {
        const a = document.createElement('a');
        document.body.appendChild(a);
        a.setAttribute('style', 'display:none');
        a.setAttribute('href', objectUrl);
        a.setAttribute('download','驾驶员评分统计报表' + this.Time);
        a.click();
  ...
  ngOnDestroy() {
    this.rows = [];
    this.list = [];
    if (this._getDriverRank) {
      this._getDriverRank.unsubscribe();
    }
    if (this._download) {
