...
export class DetailComponent extends MitDataTableBase implements OnInit, OnDestroy {
  ...
  getList(pageIndex?) {
    ...
    this.electricFenceService.GetPageRelatedVehicles(this.query).subscribe(
      (res) => {
        if (res.State) {
  ...
  GetSpeedFenceSettingByFenceId(data) {
    this.electricFenceService.GetElectronicFenceSettingByFenceId(data).subscribe(
      (res) => {
        if (res.State) {
          this.ElectricRenderInfo = res.Data;
  ...
  // 发起删除请求
  DeleteFence(data) {
    this.electricFenceService.DeleteFenceVehicles(data).subscribe((res) => {
