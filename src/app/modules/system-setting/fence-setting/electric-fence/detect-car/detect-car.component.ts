...
export class DetectCarComponent extends MitDataTableBase implements OnInit, OnDestroy {
  ...
  getList() {
    ...
    this._getList_ = this.electricFenceService.GetPageVehiclesWithoutCurFence(this.query).subscribe(
      (res) => {
        if (res.State) {
  ...
  AddFenceVehicles(data) {
    this._addFenceVehicles = this.electricFenceService.AddFenceVehicles(data).subscribe(
      (res) => {
        if (res.State) {
  ...
  ngOnDestroy() {
    ...
    if (this._addFenceVehicles) {
      this._addFenceVehicles.unsubscribe();
    }
    this.rows = [];
    this.list = [];
  }
