...
export class DetectCarComponent extends MitDataTableBase implements OnInit, OnDestroy {
  ...
  getList() {
    ...
    this._getList_ = this.nameOfCarService.GetPageVehiclesWithoutCurFence(this.query).subscribe(
      (res) => {
        if (res.State) {
  ...
  AddFenceVehicles(data) {
    this._addFenceVehicles = this.nameOfCarService.AddFenceVehicles(data).subscribe(
      (res) => {
        if (res.State) {
