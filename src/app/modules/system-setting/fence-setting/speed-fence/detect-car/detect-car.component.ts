...
export class DetectCarComponent extends MitDataTableBase implements OnInit ,OnDestroy{
  ...
  getList() {
    ...
    this._getList = this.speedFenceService.GetPageVehiclesWithoutCurFence(this.query).subscribe(
      (res) => {
        if (res.State) {
  ...
  AddFenceVehicles(data) {
    this._addFenceVehicles = this.speedFenceService.AddFenceVehicles(data).subscribe(
      (res) => {
        if (res.State) {
