...
export class ListComponent extends MitDataTableBase implements OnInit, OnDestroy {
  ...
  // 获取所有数据
  getList() {
    this._getList = this.regionFenceService.GetPageAreaDataList(this.query).subscribe(
      (res) => {
        if (res.State) {
  ...
  download(KeyValue, startTime, endTime) {
    ...
    this.text = '导出中...';
    this.isModal = true;
    this._download = this.regionFenceService.GetGetPageAreaDataListForExport(data).subscribe((res) => {
      const blob = new Blob([res], {type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"});  
      const objectUrl = URL.createObjectURL(blob); 
      if (window.navigator.msSaveOrOpenBlob) {
        navigator.msSaveBlob(blob, '区域栅栏列表');
      } else {
        const a = document.createElement('a');
        document.body.appendChild(a);
        a.setAttribute('style', 'display:none');
        a.setAttribute('href', objectUrl);
        a.setAttribute('download','区域栅栏列表');
        a.click();
