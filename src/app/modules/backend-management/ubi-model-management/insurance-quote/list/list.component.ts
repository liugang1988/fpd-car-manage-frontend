...
export class ListComponent extends MitDataTableBase implements OnInit, OnDestroy {
  ...
  getList() {
    // this.getRenderList = this.insuranceQuoteService.InsuranceCompanyList(this.query).subscribe((res) => {
    //   if ( res.State ) {
  ...
  updateStatus(id, status) {
    ...
    //   status: status ? 1 : 0
    // };
    // this.insuranceQuoteService.UpdateInsuranceCompanyStatus(data).subscribe((res) => {
