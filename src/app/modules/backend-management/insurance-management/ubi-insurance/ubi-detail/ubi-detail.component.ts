...
export class UbiDetailComponent implements OnInit, OnDestroy {
  ...
  // 获取详情
  getDetail(data){
    this._ubiInsuranceDetail = this.ubiInsuranceService.GetTrackUBIInsurance(data).subscribe((res)=>{
      if(res.Data){
  ...
  GetTrackTrajectory(TrackId) {
    this.ubiInsuranceService.TrackTrajectory(TrackId).subscribe((res) => {
      if (res.State) {
  ...
  // 取消
  back() {
    this.router.navigate(['../../ubi-list/'+ this.TrackLineData.TrackInfo.VID], { relativeTo: this.activatedRoute });
  }
